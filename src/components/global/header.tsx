...
/**
 * The navbar  with a ```localStorage``` cached theme setting and controller.
 * 
 * If this functionality is removed/ignored - will default to following system light/dark setting
 * 
 * @returns A generic navbar
 */

const Header = () => {
  const [theme, setTheme] = useState(localStorage.getItem("theme") || "light");

  const themeToggle = (isChecked: boolean) => {
    if (isChecked) {
      setTheme("dark");
    } else {
      setTheme("light");
  ...
  return (
    <>
      <header>
        <nav className="navbar fixed z-10 bg-base-300 shadow-md">
          <Menu
            links={[
              { name: "Homepage", href: "/" },
              {
          ...
          <label className="navbar-end cursor-pointer pr-3">
            <input
              type="checkbox"
              className="hidden"
              onChange={(e) => themeToggle(e.currentTarget.checked)}
            />

            {theme === "dark" ? (
              <>
                <p className="bold pr-3">Light</p>
                <svg
